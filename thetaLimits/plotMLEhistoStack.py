from ROOT import TFile, TH1D, THStack, TPad, TLegend, TLatex, TGraphAsymmErrors, gROOT
...
mleFile = "limitsJune2020/templatesSRCR_TT_mle/histos-mle_"+mleType
outdir = 'limitsJune2020/templatesSRCR_TT_mle/postfitplots_'+mleType
...
def formatTOPPlot(hTOP):
	hTOP.SetLineColor(ROOT.kAzure+8)
	hTOP.SetFillColor(ROOT.kAzure+8)
	hTOP.SetLineWidth(2)

def formatEWKPlot(hEWK):
	hEWK.SetLineColor(ROOT.kMagenta-2)
	hEWK.SetFillColor(ROOT.kMagenta-2)
	hEWK.SetLineWidth(2)

def formatQCDPlot(hQCD):
	hQCD.SetLineColor(ROOT.kOrange+5)
	hQCD.SetFillColor(ROOT.kOrange+5)
	hQCD.SetLineWidth(2)
